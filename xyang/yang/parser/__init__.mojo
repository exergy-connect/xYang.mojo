...
from xyang.yang.parser.grouping_registry import (
    store_grouping_impl,
    append_grouping_nodes_by_name_impl,
    find_grouping_index_impl,
)
...
from xyang.yang.parser.types import YangToken, ParsedGrouping, ParsedAugment
...
struct _YangParser(Movable, ParserContract):
    ...
    def _store_grouping(mut self, var grouping: ParsedGrouping) raises:
        ...
        if not store_grouping_impl(self.grouping_names, self.groupings, grouping^):
    ...
    def _append_grouping_nodes_by_name(
        ...
        if not append_grouping_nodes_by_name_impl(
            self.grouping_names,
            self.groupings,
            grouping_name,
            leaves,
    ...
    def _find_grouping_index(ref self, grouping_name: String) -> Int:
        return find_grouping_index_impl(self.grouping_names, grouping_name)
