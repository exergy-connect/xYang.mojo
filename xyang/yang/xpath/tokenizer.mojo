...
def _scan_one_token(mut self: XPathTokenizer) raises -> Token:
    ...
    if c == CP_PAREN_OPEN:
        _advance(self)
        return Token(
            type=Token.PAREN_OPEN, start=start, length=1, line=line_start
        )
    if c == CP_PAREN_CLOSE:
        _advance(self)
        return Token(
            type=Token.PAREN_CLOSE, start=start, length=1, line=line_start
        )
    if c == CP_BRACKET_OPEN:
        _advance(self)
        return Token(
            type=Token.BRACKET_OPEN, start=start, length=1, line=line_start
        )
    if c == CP_BRACKET_CLOSE:
        _advance(self)
        return Token(
            type=Token.BRACKET_CLOSE, start=start, length=1, line=line_start
        )
    if c == CP_COMMA:
        _advance(self)
    ...
    if c == CP_EQUALS:
        _advance(self)
        return Token(
            type=Token.OPERATOR, start=start, length=1, line=line_start
        )
    if c == CP_BANG and _peek_next(self) == CP_EQUALS:
        _advance(self, 2)
        return Token(
            type=Token.OPERATOR, start=start, length=2, line=line_start
        )
    if c == CP_LT:
        _advance(self)
