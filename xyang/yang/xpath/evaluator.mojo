...
struct XPathEvaluator(ExprEvalVisitor):
    ...
    def _compare_left_against_list(
        ...
        if self._is_comma_list(list_node, ctx.expression):
            return self._compare_left_against_list(
                left_value, list_node.left[], ctx, current
    ...
    def visit_binary(
        ...
        if op == "or":
            ...
            return EvalResult(
                _yang_bool(eval_accept(self, node.right[], ctx, current))
            )
        if op == "and":
            var left = eval_accept(self, node.left[], ctx, current)
        ...
        if op == "=":
            ...
            if self._is_comma_list(node.right[], ctx.expression):
                return EvalResult(
                    self._compare_left_against_list(
                        left, node.right[], ctx, current
                    )
                )
        ...
        if op == "!=":
            ...
            if self._is_comma_list(node.right[], ctx.expression):
                return EvalResult(
                    not self._compare_left_against_list(
                        left, node.right[], ctx, current
                    )
                )
            var right = eval_accept(self, node.right[], ctx, current)
    ...
    def visit_path(
        ...
        for i in range(len(node.steps)):
            ...
            for k in range(len(step_expr.args)):
                nodes = self._apply_predicate(
                    nodes, step_expr.args[k][], path_ctx
                )
    ...
    def visit_call(
        ...
        if name == "string-length":
            if len(node.args) == 1:
                var a = eval_accept(self, node.args[0][], ctx, current)
                return EvalResult(
                    Float64(len(_result_to_string(_first_value(a^))))
                )
            return EvalResult(0.0)
        return EvalResult(0.0)
