...
struct Path(Copyable):
    ...
    def __init__(
        out self,
        absolute: Bool = False,
        var segments: List[QName] = List[QName](),
    ):
...
def parse_qname(segment: String) raises -> QName:
    """Parse a single ``node-identifier`` (``prefix:local`` or ``local``)."""
...
def local_names_from_slashed_path_parts(
    read parts: List[String],
) raises -> List[String]:
...
def _qnames_from_slashed_path_parts(
    read parts: List[String],
) raises -> List[QName]:
    var out = List[QName]()
    for i in range(len(parts)):
...
def parse_path(expression: StringSlice) raises -> Path:
    ...
    return Path(
        absolute=absolute, segments=_qnames_from_slashed_path_parts(parts)
    )
